...
public class OrderController {
    ...
    public ResponseEntity<?> getMyAccepted(@RequestHeader("X-User-Id") Long currentUserId,
                                           @RequestParam(required = false) String status) {
        java.util.List<Map<String, Object>> history = orderService.getAcceptHistory(currentUserId, status);
        return ResponseEntity.ok(buildResponse(200, "查询接单历史成功", history));
    }
}